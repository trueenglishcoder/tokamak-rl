...
from tokamak_rl.training.cli import _make_env_factory, _make_eval_env_factory, _resolve_eval_num_envs
...
def test_gpu_eval_uses_dense_batch_size(tmp_path: Path) -> None:
    config_path = tmp_path / "small_sim.toml"
    _write_small_sim_config(config_path)
    experiment = _Experiment(
        EnvConfig(
    ...
    eval_num_envs = _resolve_eval_num_envs(compute_backend="gpu", requested_num_envs=64, eval_episodes=16)

    factory = _make_eval_env_factory(
    ...
    assert isinstance(factory, TrueBatchedGpuEnvFactory)
    assert factory.num_envs == 16


def test_cpu_eval_keeps_single_env_size() -> None:
    assert _resolve_eval_num_envs(compute_backend="cpu", requested_num_envs=64, eval_episodes=16) == 1
